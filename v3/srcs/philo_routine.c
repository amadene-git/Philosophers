...
/*   Updated: 2021/10/06 15:31:36 by admadene         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*routine_philo_odd(void *data)
{
	...
	!philo->info->is_die)
	{
		...
		if (philo->id == philo->info->i)
		{
			printf("yo -> %d\n", philo->id + 1);
			pthread_mutex_lock(&philo->info->mutex_a);
			printf("wjden\n");
			if (philo->info->i + 2 >= philo->info->nbr_philo)
				philo->info->i = 0;
			else
				philo->info->i += 2;
		}
...
void	*routine_philo_even(void *data)
{
	...
	!philo->info->is_die)
	{
		...
		if (philo->info->i == -1 && philo->id == philo->info->i + 1)
		{
			printf("yo -> %d\n", philo->id + 1);
			pthread_mutex_lock(&philo->info->mutex_a);
			printf("wjden\n");
